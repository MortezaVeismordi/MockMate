...
class InterviewConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not self.session:
            logger.warning(
                ...
                self.session_uuid, self.user.pk,
            )
    ...
    async def disconnect(self, close_code: int):
        ...
        logger.info(
            ...
            getattr(self, "session_uuid", "unknown"), close_code,
        )
    ...
    async def receive(self, text_data: str):
        ...
        try:
            ...
            logger.error(
                ...
                self.session_uuid, event_type, str(exc),
                exc_info=True,
            )
    ...
    async def _handle_start(self, payload: dict):
        ...
        await self._send_event("greeting", {
    ...
    async def _handle_submit_answer(self, payload: dict):
        ...
        # اعلام دریافت پاسخ
        await self._send_event("answer_received", {
            "answer_id": answer.pk,
            "status"   : answer.status,
        })

        # اعلام شروع ارزیابی
        await self._send_event("evaluating", {
    ...
    def _ask_next_question(self):
        ...
        return {
            ...
            "question_type" : result.related_question.question.question_type
                              if result.related_question else "",
            "code_template" : result.related_question.question.code_template
                              if result.related_question else None,
        }
    ...
    def _submit_follow_up_answer(self, answer_text: str):
        from .services import InterviewConductService
        InterviewConductService.submit_follow_up_answer(
            self.session, answer_text
        )
