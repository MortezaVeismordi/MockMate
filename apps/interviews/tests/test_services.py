...
class TestInterviewServices(TransactionTestCase):
    ...
    def test_session_creation_fails_with_active_session(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            InterviewSetupService.create_session(user=self.user, target_position="Dev2", seniority_level="mid_level")
    ...
    def test_eval_service_success(self):
        ...
        with patch('apps.core.llm.client.LLMClient.evaluate', return_value=mock_eval_result):
    ...
    @patch('apps.core.llm.client.LLMClient.generate_report_summary', return_value="Great job")
    def test_report_service_aggregates_results(self, mock_report_gen):
        session = InterviewSessionFactory.create(user=self.user, target_position="Dev", status=InterviewSession.Status.WRAP_UP)
        q1 = Question.objects.all()[0]
        q2 = Question.objects.all()[1]
        
        UserAnswerFactory.create(session=session, user=self.user, question=q1, score=80, status=UserAnswer.Status.GRADED)
        UserAnswerFactory.create(session=session, user=self.user, question=q2, score=90, status=UserAnswer.Status.GRADED)
